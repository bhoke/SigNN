...
from mydataset import SignatureTrain, SignatureTest
...
if __name__ == '__main__':
    ...
    trainSet = SignatureTrain(Flags.data_path, transform=data_transforms)
    testSet = SignatureTest(Flags.data_path, transform=transforms.ToTensor(), times = Flags.times, way = Flags.way)
    ...
    for batch_id, (img1, img2, label) in enumerate(trainLoader, 1):
        ...
        if batch_id % Flags.show_every == 0 :
            print('[%d]\tloss:\t%.5f\ttime elapsed:\t%.2f s'%(batch_id, loss_val/Flags.show_every, time.time() - time_start))
            loss_val = 0
            time_start = time.time()
